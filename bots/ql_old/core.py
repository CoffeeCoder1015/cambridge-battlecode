...
class Core:
    ...
    def run(self, ct: Controller):
        ...
        if ct.get_action_cooldown() == 0:
            ...
            # 2. Unlimited spending & Flow Reinvestment
            BUDGET_PER_BOT = 120  # Harvester + Conveyer
            if titanium / max(1, self.spawned) > BUDGET_PER_BOT and titanium >= 20:
                print(f"[Core] Unlimited Spawn. Ti: {titanium}, Bots: {self.spawned}", file=sys.stderr)
                self._spawn(ct)
            elif self.reinvestment_budget >= 132 and titanium >= 20:
