...
class BridgeBuilder:
    ...
    def main(
        ...
        if self._resume_random_after_bridge:
            ...
            if self._start_remembered_ore_navigation(ct):
                return self._advance_remembered_ore_navigation(ct)
            self._log(
                ct, "bridge cycle complete at core, re-entering random exploration"
            )
            return self._run_random_fallback(ct)

        visible_ores = self._visible_ores_from_scan(ct, symmetry_analyzer)
        self._log(
            ct, f"visible ores in scan={len(visible_ores)} {sorted(visible_ores)}"
        )
        ...
        if move_dir is None:
            self._log(
                ct,
                f"could not bugnav path to ore {self.ore_target}, returning to random",
            )
            self._clear_primary_ore_target()
            return self._run_random_fallback(ct)
    ...
    def _visible_ores_from_scan(
        self, ct: Controller, symmetry_analyzer: Any | None
    ) -> set[tuple[int, int]]:
        ...
        self._log(
            ct, "symmetry map_history unavailable, using nearby tile scan fallback"
        )
    ...
    def _build_generator_on_ore(self, ct: Controller, ore_pos: Position) -> bool:
        ...
        if self._ore_has_completed_extractor(ct, ore_pos):
            self._log(
                ct, f"ore already has harvester/generator at ({ore_pos.x},{ore_pos.y})"
            )
            return False
        ...
        if ct.get_action_cooldown() != 0:
            self._log(
                ct, f"build blocked by action cooldown={ct.get_action_cooldown()}"
            )
            return False

        affordable_extractor, extractor_cost, available_resources = (
            get_cost_affordability(
                ct,
                "get_harvester_cost",
            )
        )
        ...
        if callable(can_build_generator) and callable(build_generator):
            ...
            self._log(
                ct,
                f"generator API available but cannot build at ({ore_pos.x},{ore_pos.y})",
            )
    ...
    def _run_post_generator_bridge(
        ...
        if ct.get_action_cooldown() != 0:
            self._log(
                ct,
                f"bridge build blocked by action cooldown={ct.get_action_cooldown()}",
            )
            return True
        ...
        if ct.can_build_bridge(start_pos, target_pos):
            ...
            self._log(
                ct,
                f"built bridge from ({start_pos.x},{start_pos.y}) to ({target_pos.x},{target_pos.y})",
            )
    ...
    def _advance_post_bridge_navigation(
        ...
        my_pos = ct.get_position()
        self._log(
            ct, f"post-bridge nav at ({my_pos.x},{my_pos.y}) -> target ({tx},{ty})"
        )
        ...
        if (new_pos.x, new_pos.y) == (tx, ty):
            self._log(
                ct,
                "arrived at bridge target after movement; triggering next bridge cycle step",
            )
            self._post_bridge_target = None
            self._post_bridge_nav.reset()
            self._post_generator_bridge_pending = True
    ...
    def _start_remembered_ore_navigation(self, ct: Controller) -> bool:
        ...
        self._log(
            ct, f"bridge cycle complete at core, bugnav to remembered ore {(tx, ty)}"
        )
        return True
    ...
    def _advance_remembered_ore_navigation(self, ct: Controller) -> bool:
        ...
        if self._ore_has_completed_extractor(ct, ore_pos):
            self._log(
                ct,
                f"remembered ore ({tx},{ty}) already harvested, aborting remembered nav",
            )
            self._remembered_ore_target = None
        ...
        if self._in_action_radius(my_pos, ore_pos):
            ...
            if built:
                ...
                if blocking_type is not None:
                    ...
                    return self._run_random_fallback(ct)
                self._log(
                    ct, "generator build on remembered ore not possible this turn"
                )
            return True
    ...
    def _run_post_build_cardinal_alignment(self, ct: Controller) -> bool:
        ...
        self._log(
            ct,
            f"post-build alignment waiting at ({my_pos.x},{my_pos.y}) for ore ({ox},{oy})",
        )
        return True

    def _move_to_cardinal_adjacent_tile(
        self, ct: Controller, ore_pos: Position
    ) -> bool:
        my_pos = ct.get_position()
        for move_dir in CARDINAL_DIRECTIONS:
            ...
            if acted:
                self._log(
                    ct, f"post-build alignment securing cardinal tile ({nxt.x},{nxt.y})"
                )
                return True
    ...
    def _finish_bridge_cycle_to_existing_return_path(
        self, ct: Controller, target_pos: Position
    ) -> None:
        self._log(
            ct,
            (
